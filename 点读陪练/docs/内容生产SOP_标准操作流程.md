...
### 步骤 10b：发布粒度与备份方案（既定决策，2026-09-25）

**既定粒度：每册 2 单元一包（U1-02）**——维持现状，**不合并、不拆单单元**。

| 决策 | 结论 | 理由 |
|---|---|---|
| 合并粒度 | **每册 U1-02 一包** | ① 体积控制：带歌 2 单元 ≈ 4.6-8.4MB，全册 6 单元带歌 ≈ 24MB 超限；全无歌全册虽 ~6.5MB 可行，但保持粒度统一便于控制 ② **免费试听定位**：小红书 U1/U2 作为在线版收费前的免费试听入口，2 单元一包粒度利于引导付费转化（在线版收费后 U1/U2 继续免费） |
| 全册合并（六单元一包） | ❌ 否决 | 评估过（无歌册 ~6.5MB / ~140 项，双约束可过），但增加单包控制复杂度，与试听定位不符 |
| 歌曲压缩（备份方案） | 🔒 备用不实施 | 见下「歌曲压缩备份方案」；已发带歌包（一上/三上/四上）不重发 |

**歌曲压缩备份方案**（备用，触发条件：补歌后 2 单元包逼近 10MB 临界 ~9.7MB，或需进一步降体积时启用）：

- **方案 C（推荐）**：`src/publish_offline.py` `compress_song()` 的 ffmpeg 参数改为——人声轨 `-ac 1 -b:a 96k`、伴奏轨 `-ac 1 -b:a 64k`（当前为立体声 128k）
- **收益**：每首歌 base64 从 ~3.0MB → ~1.9MB（省 38%；极端可统一 64k mono 省 50%）
- **兼容性**：mp3 单声道 `decodeAudioData` 直接解，**前端零改动**；教学跟唱场景手机外放听感可接受
- **启用步骤**：改参数 → **删除 `build/<unit>/assets/song/` 旧 mp3**（压缩脚本按 mtime 跳过新鲜文件）→ 重新 `publish_offline.py` 发布该单元
- **现状**：已发带歌包（一上/三上/四上）维持 128k 立体声不重发；仅对未来补歌单元生效
...
**粒度**：每册 U1-02 一包（既定决策，见步骤 10b——不合并全册、不拆单单元；小红书 U1/U2 为在线版收费前免费试听入口）。
