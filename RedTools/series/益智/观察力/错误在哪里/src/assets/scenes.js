...
(function () {
  ...
  /* ============================================================
     错误在哪里 — 错位候选表（A1 专属）
     ------------------------------------------------------------
     每个场景 3 个「外部错位候选」：从其它场景借 1 个元素嵌入本场景，
     形成不合逻辑的异常（如海边出现兔子、森林出现章鱼）。
     - 结构: { 场景id: [ { srcScene, elemId, hint, x, y } ] }
       srcScene: 源场景对象（元素库内引用）
       elemId  : 源场景中的元素 id（其 draw 渲染）
       hint    : 提示文案（家长引导用，不直接显示）
       x/y     : 错位元素在本场景的渲染位置（视口坐标）
     - 渲染: main.js 取候选元素 -> 用 srcScene 元素 draw 渲染嵌入本场景
     ============================================================ */
  var sceneByName = {};
  [ocean, forest, farm, space, undersea, city, winter, garden].forEach(function (s) {
    sceneByName[s.id] = s;
  });
  var elemByKey = {};
  [ocean, forest, farm, space, undersea, city, winter, garden].forEach(function (s) {
    s.elements.forEach(function (e) { elemByKey[s.id + '.' + e.id] = e; });
  });

  window.WRONG_OPTIONS = {
    ocean: [
      { srcScene: 'forest', elemId: 'rabbit', hint: '海边怎么会有兔子呢？', x: 180, y: 235 },
      { srcScene: 'undersea', elemId: 'octopus', hint: '章鱼不应该在沙滩上呀', x: 390, y: 150 },
      { srcScene: 'space', elemId: 'rocket', hint: '火箭怎么飞到海边来了？', x: 90, y: 40 }
    ],
    forest: [
      { srcScene: 'undersea', elemId: 'octopus', hint: '森林里会有章鱼吗？', x: 360, y: 210 },
      { srcScene: 'winter', elemId: 'snowman', hint: '夏天的森林不该有雪人', x: 120, y: 255 },
      { srcScene: 'space', elemId: 'ufo', hint: '树林里怎么有飞碟？', x: 300, y: 40 }
    ],
    farm: [
      { srcScene: 'undersea', elemId: 'whale', hint: '鲸鱼怎么游到农场了？', x: 150, y: 90 },
      { srcScene: 'space', elemId: 'astronaut', hint: '宇航员来农场做什么？', x: 360, y: 150 },
      { srcScene: 'winter', elemId: 'penguin', hint: '农场里有企鹅吗？', x: 80, y: 255 }
    ],
    space: [
      { srcScene: 'farm', elemId: 'cow', hint: '太空里怎么有奶牛？', x: 200, y: 120 },
      { srcScene: 'undersea', elemId: 'fish2', hint: '鱼怎么在太空游？', x: 380, y: 200 },
      { srcScene: 'garden', elemId: 'flower2', hint: '太空中会长花吗？', x: 90, y: 250 }
    ],
    undersea: [
      { srcScene: 'space', elemId: 'rocket', hint: '火箭怎么沉到海底了？', x: 340, y: 80 },
      { srcScene: 'farm', elemId: 'cow', hint: '海底怎么会有奶牛？', x: 120, y: 230 },
      { srcScene: 'winter', elemId: 'snowman', hint: '海底的雪人不会化吗？', x: 420, y: 150 }
    ],
    city: [
      { srcScene: 'undersea', elemId: 'whale', hint: '鲸鱼怎么游到城市里了？', x: 100, y: 60 },
      { srcScene: 'winter', elemId: 'penguin', hint: '城市马路上有企鹅吗？', x: 350, y: 210 },
      { srcScene: 'winter', elemId: 'deer', hint: '鹿怎么跑到马路上？', x: 200, y: 250 }
    ],
    winter: [
      { srcScene: 'ocean', elemId: 'palm', hint: '冰天雪地怎么会有椰子树？', x: 380, y: 250 },
      { srcScene: 'undersea', elemId: 'octopus', hint: '雪地里怎么有章鱼？', x: 90, y: 150 },
      { srcScene: 'garden', elemId: 'butterfly2', hint: '冬天会有蝴蝶吗？', x: 240, y: 60 }
    ],
    garden: [
      { srcScene: 'space', elemId: 'astronaut', hint: '花园里怎么有宇航员？', x: 350, y: 80 },
      { srcScene: 'undersea', elemId: 'whale', hint: '鲸鱼怎么游到花园里？', x: 120, y: 50 },
      { srcScene: 'winter', elemId: 'snowman', hint: '花园里不该有雪人吧', x: 200, y: 240 }
    ]
  };
  window.SCENE_BY_NAME = sceneByName;
  window.ELEM_BY_KEY = elemByKey;
})();
