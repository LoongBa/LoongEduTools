...
(function () {
  ...
  /* ---------- 形状小工具 ---------- */
  /* c() 双签名兼容：c(r, f) 原点圆 | c(cx, cy, r, f) 指定圆心（元素库两式混用） */
  function c(a1, a2, a3, a4) {
    if (a3 !== undefined) {
      return '<circle cx="' + a1 + '" cy="' + a2 + '" r="' + a3 + '" fill="' + a4 + '"/>';
    }
    return '<circle cx="0" cy="0" r="' + a1 + '" fill="' + a2 + '"/>';
  }
  ...
  /* 中文名表（与找相同/划消/影子配对 elements.js 的 ELEM_NAMES 同源） */
  window.ELEM_NAMES = {
