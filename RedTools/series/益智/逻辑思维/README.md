...
> 产品排期见 `docs/教育工具/教育工具产品矩阵规划.md` §3.4（逻辑思维训练：数独侦探 P1 / 迷宫寻路 P1 / 分类整理 P1 / 推箱子 P2 / 因果排序 P2）。
> 更新：2026-09-19
...
| 数独侦探 | P1 | ✅ 已构建 | v1.0 | 程序化出题零题库（回溯终盘 + 挖洞唯一解，4×4/6×6/9×9 三档，9×9 生成 5ms）；点格选数 + 同行列宫/同数字高亮；橡皮/撤销/智能提示；即时反馈（冲突红闪）；星级（提示×错误）；各难度最佳/打卡 |
...
python build_all.py --tool 数独侦探   # 构建数独侦探
...
- **✅ 全部完成**：逻辑思维子系列产品矩阵 6 款（迷宫寻路 P1 / 分类整理 P1 / 数独侦探 P1 / 推箱子 P2 / 因果排序 P2 / 蜡笔物理 P2）已全部交付（2026-09-19，子系列收官）。
