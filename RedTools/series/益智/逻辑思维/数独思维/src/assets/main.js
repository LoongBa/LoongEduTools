...
/* ---------- 成就（v1.12 X3 / v1.17 扩 2 / v1.18 扩 2 / v1.20 扩 1 / v1.21 扩 3 / v1.22 扩 1 / v1.24 扩 1）：16 枚个人里程碑，无竞技无排行 ---------- */
  var ACHIEVEMENTS = [
    ...
    { key: 'sixArts',    name: '六艺精通', desc: '点亮全部 16 枚技巧徽章（适龄 4 + 进阶 12）' },  // v1.20/v1.23/v1.25/v1.26/v1.27/v1.28/v1.29/v1.30/v1.31/v1.32/v1.33
...
function baseSkillCount() {
  ...
  var SKILLS = [
    ...
    // 隐性数对（v1.25，进阶 adv）：第 2 列数字 1/2 仅出现在 idx14/26（隐性数对）→ 锁定后数字 5 只剩 idx32（唯一解已验证：idx32=5、idx14=1）
    {
      key: 'hiddenPair',
      name: '隐性数对',
      group: 'adv',
      tip: '隐性数对：两个数字都只能放在同样的两格 → 这两格就是那两个数，其它候选数都排除',
      N: 6,
      ...
      done: '真棒！你学会了「隐性数对」——两个数字悄悄藏在同一两格里，其它候选数就被排除啦！'
    },
    // 三数组（v1.26，进阶 adv）：宫 3 内格 17/21/23 候选并集恰 {1,4,6}（每格 ⊆{1,4,6}）三数组占位 → 排除后 idx15 填 2、idx16 填 5（唯一解已验证：idx15=2、idx16=5）
    {
      key: 'nakedTriple',
      name: '三数组',
      group: 'adv',
      tip: '三数组占位：一个行、列或宫里，三个格都只能填同样的三个数 → 其它格就不能再填它们了',
      N: 6,
      ...
      done: '真棒！你学会了「三数组」——三个格锁住同样的三个数，其它格就知道排除它们了！'
    },
    // 隐性三数组（v1.27，进阶 adv）：宫 1 内数字 1/2/3 候选位 ⊆ 格 3/5/9（隐性三数组）→ 锁定后数字 4 只剩 idx4、数字 6 只剩 idx10（唯一解已验证：idx4=4、idx10=6）
    {
      key: 'hiddenTriple',
      name: '隐性三数组',
      group: 'adv',
      tip: '隐性三数组：三个数字都只能放在同样的三格 → 这三格就是那三个数，其它候选数都排除',
      N: 6,
      ...
      done: '真棒！你学会了「隐性三数组」——三个数字悄悄藏在同一三格里，其它候选数就被排除啦！'
    },
    // XY-Wing（v1.28，进阶 adv）：pivot idx23 {6,3} + wing1 idx5 {6,4} + wing2 idx11 {3,4} → 两翼必含 4 → 共同 peer idx3 排除 4 填 5；step2 新 pivot idx5 {4,6} 排除 3 后 idx11 填 4（唯一解已验证：idx3=5、idx11=4）
    {
      key: 'xyWing',
      name: 'XY-Wing',
      group: 'adv',
      tip: 'XY-Wing：一个尖格连着两个翅膀，两个翅膀里必有一个数 → 它们共同看到的格子就能排除它',
      N: 6,
      ...
      done: '真厉害！你学会了「XY-Wing」——用尖格和翅膀一起推理，其它格子就知道排除它了！'
    },
    // 剑鱼（v1.29，进阶 adv）：数字 3 在第 1/2/6 行候选列 ⊆ {1,3,5}（3 行 3 列剑鱼）→ 3 列内其它行排除 3 → idx24 填 5、idx28 填 4（唯一解已验证：idx24=5、idx28=4）
    {
      key: 'swordfish',
      name: '剑鱼',
      group: 'adv',
      tip: '剑鱼：一个数字在三行都被锁在同样的三列 → 这三列其它行就能排除它',
      N: 6,
      ...
      done: '真厉害！你学会了「剑鱼」——一个数字被三行三列锁住，其它格子就知道排除它了！'
    },
    // 唯一矩形（v1.30，进阶 adv）：UR1 矩形 [2,4,8,10] 三格 {2,3} + 第四格 {2,3,6} → 必填 6；UR2 矩形 [6,7,12,13] 三格 {3,5} + 第四格 {1,3,5} → 必填 1（唯一解已验证：idx2=6、idx7=1）
    {
      key: 'uniqueRect',
      name: '唯一矩形',
      group: 'adv',
      tip: '唯一矩形：四个格子组成矩形，如果填成双解陷阱就会出现两个答案 → 只能填剩下的那个数',
      N: 6,
      ...
      done: '真厉害！你学会了「唯一矩形」——四个格子不能填成双解陷阱，答案就露出来了！'
    },
    // W-Wing（v1.31，进阶 adv）：A/B 候选同 {5,6} 异单元 + x=5 三强链（A 行 C、B 行 D、C/D 同列）→ A/B 不同时 5 → 共同 peer 排除 6 → idx3 填 3、idx16 填 3（唯一解已验证：idx3=3、idx16=3）
    {
      key: 'wWing',
      name: 'W-Wing',
      group: 'adv',
      tip: 'W-Wing：两个格都只能填同样的两个数，中间夹着链条 → 它们共同看到的格子排除一个数',
      N: 6,
      ...
      done: '真厉害！你学会了「W-Wing」——两个格和一条链，就能排除共同看到的格子！'
    },
    // 链与着色（v1.32，进阶 adv，§2.19 演示关降级）：数字 1 候选格 strong-link 链染 A/B 色，同色格 0/11/29/32 中 11/29 同列冲突 → 该色全排除 1 → idx0 填 2（唯一解已验证：idx0=2）
    {
      key: 'chains',
      name: '链与着色',
      group: 'adv',
      tip: '链与着色：数字的候选格手拉手连成链，交替染色——如果同色格子打架，这种颜色就全排除',
      N: 6,
      ...
      done: '真厉害！你学会了「链与着色」——给链上的格子染色，打架的颜色就排除它！'
    },
    // 强制链（v1.33，进阶 adv，§2.19 演示关降级）：双候选格 idx1 {1,3} → 分支 1 传播 → idx3=5；分支 3 传播 → idx3=5（两分支同结论）→ idx3 无条件填 5（唯一解已验证：idx3=5）
    {
      key: 'forcing',
      name: '强制链',
      group: 'adv',
      tip: '强制链：一个格子试两种可能，两种可能推理到最后都指向同一个答案 → 那就是答案',
      N: 6,
      ...
      done: '真厉害！你学会了「强制链」——两种可能都通向同一个答案，那答案就是它！'
    }
  ];
  var skill = {
    active: false,
