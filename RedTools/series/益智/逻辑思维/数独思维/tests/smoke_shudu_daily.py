...
用例：
  A. 难度页：📅 每日挑战卡（未完成）+ 🏅 成就卡（0/15）
  B. 每日挑战开局：当日难度盘面（v1.17 周轮换 4/6/9）+ 顶栏「· 每日题」
  C. 通关 → store.daily 写入 + 结算浮层「🏅 解锁成就：每日挑战首通」
  D. 返回难度：每日卡变「今日已完成 ✅」+ 成就卡（2/15）
  E. 成就视图：15 卡（🏅 2 解锁 + 🔒 13 未解锁）+ 解锁日期
...
def main():
    with sync_playwright() as p:
        ...
        check("A3 成就卡存在 0/15", "0 / 15" in page.locator("button:has-text('成就') .teach-btn-sub").inner_text())
        ...
        check("D2 成就卡 2/15（首通+三星完美）", "2 / 15" in page.locator("button:has-text('成就') .teach-btn-sub").inner_text())

        # E. 成就视图
        page.locator("button:has-text('成就')").click()
        page.wait_for_timeout(250)
        ach_cards = page.locator(".teach-btn.badge-card")
        check("E1 成就卡 15 张", ach_cards.count() == 15)
        check("E2 已解锁 🏅 2 枚",
              page.locator(".teach-btn.badge-card .teach-btn-head", has_text="🏅").count() == 2)
        check("E3 未解锁 🔒 13 枚",
              page.locator(".teach-btn.badge-card .teach-btn-head", has_text="🔒").count() == 13)
