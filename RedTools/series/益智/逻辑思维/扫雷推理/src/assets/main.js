...
(function () {
  ...
  /* ---------- 常量 ---------- */
  var STORE_KEY = 'redtools.saoleituili.v1';   // D2 改造：独立键名（扫雷推理）
  LX_SHARED.storage.configure({ toolName: 'saoleituili' });  // V0.4 迁移：键前缀 redtools.saoleituili.v1
  ...
  var state = {
    ...
    missCount: 0,           // D2：踩雷次数（不判负，仅提示+影响展示）
    hitMines: []            // D2：已踩雷集合（显示 💣 标记，不再触发终局）
  };
  ...
  function loadStore() {
    // V0.4 迁移：LX_SHARED.storage（configure 注入 → redtools.saoleituili.v1）
    var s = LX_SHARED.storage.get('v1');
    return s ? normalizeStore(s) : defaultStore();
  }
  function saveStore() {
  ...
  /* D2：踩雷推理提示（toast，非终局弹窗）——显示该雷位置 + 提示用数字推理避雷 */
  function showMineHint(boomIdx) {
    try {
      var el = document.getElementById('mine-hint');
      if (!el) {
      ...
      var msg = '💥 踩到雷了（第 ' + (state.missCount || 1) + ' 次）——已标出。' +
        '看周围数字：几就代表周围有雷，已标出的雷不用再踩。';
      el.textContent = msg;
      el.className = 'mine-hint show';
      setTimeout(function () {
        if (el) { el.className = 'mine-hint'; }
      }, 3000);
    } catch (err) { /* ignore */ }
  }

  function loseGame(boomIdx) {
    // D2 改造：踩雷不判负——标记该雷（红闪提示），局继续，可继续推理
    if (state.over || state.won) { return; }
    state.boomIdx = boomIdx;
    state.missCount = (state.missCount || 0) + 1;   // 踩雷次数（影响星级）
    if (state.hitMines.indexOf(boomIdx) < 0) { state.hitMines.push(boomIdx); }
    sndLose();
    // 显示推理提示（踩雷格 = 已确认雷，标旗化；提示数字推理）
    showMineHint(boomIdx);
    paintBoard();
    updateInfo();
    renderFooter();
    renderGuardBar();
    // 不置 over —— 继续翻开其他格
    // 若踩雷后仍有可推理空间，checkWin 仍会判定完成
    checkWin();
  }
  ...
  function paintCell(i) {
    ...
    if (opened) {
      ...
      fill.textContent = '🚩';
    } else if (state.hitMines && state.hitMines.indexOf(i) >= 0) {
      // D2：已踩雷标记（不终局，显示地雷提醒避开）
      fill.className = 'cell-fill mine boom';
      fill.textContent = '💣';
    } else {
  ...
  function openWin() {
    ...
    if (state.missCount) {
      notes.push({ cls: 'miss-line', text: '踩雷 ' + state.missCount + ' 次（已标出，下次更稳）' });
    }
    var streak = store.checkin && store.checkin.streak ? store.checkin.streak : 0;
    notes.push({
      cls: 'checkin-line',
    ...
    showOverlay('🎉 完成推理！', '所有安全格都翻开了，逻辑清晰！', state.time + 's', notes, guardSettleBtns());
  }
  function openLose() {
    // D2：主流程踩雷不判负（loseGame 不再调用本函数）；断局恢复场景提示
    var bv = store.best[state.bestKey] || 0;
    var notes = [{ cls: 'best-line', text: bv > 0 ? '本难度最佳 ' + bv + 's' : '本难度最佳 —' }];
    notes = notes.concat(guardSettleNotes());
    showOverlay('💡 推理未完', '继续翻开安全格，避开已标出的雷', state.time + 's', notes, guardSettleBtns());
  }
