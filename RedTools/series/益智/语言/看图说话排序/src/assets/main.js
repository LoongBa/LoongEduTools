/* ============================================================
   看图说话排序 — 主逻辑（ES2017 经典脚本，Chrome 61 基线）
   ------------------------------------------------------------
   数据：window.STORIES（stories.js 注入，8 条叙事序列卡片库）
         window.APP_DATA（data.js 注入，build 生成 meta，可缺失）
   玩法：打乱故事卡片按情节顺序排好（交换排序，因果排序交互模式）
   - 选难度（3/4/5 步）→ 随机取同步数叙事序列 → 洗牌打乱（防已还原重洗）
   ...
   - 计时：performance.now() + setInterval(100ms) 差值刷新（§4.1），
     从游戏页渲染开始计时（整局计时）
   - 成绩/打卡：localStorage key 带工具前缀 redtools.tushuopaixu.v1
   ============================================================ */
(function () {
  'use strict';
  ...
  /* ---------- 持久化（redtools.tushuopaixu.v1） ---------- */
  var STORE_KEY = 'redtools.tushuopaixu.v1';
  LX_SHARED.storage.configure({ toolName: 'tushuopaixu' });  // V0.4 迁移：键前缀 redtools.tushuopaixu.v1
  ...
  function renderHeader(title) {
    ...
    var name = APP.meta && APP.meta.name ? APP.meta.name : '看图说话排序';
  ...
  function showDifficultyView() {
    ...
    viewEl.appendChild(makeEl('div', 'home-hint', '看图讲故事：点两张卡片交换顺序，把故事按情节排对就过关！'));
  ...
  function pickSequence(lv) {
    ...
    if (!cands.length) {
      for (var j = 0; j < STORIES.length; j++) {
        if (STORIES[j].steps.length === lv.stepN) { cands.push(STORIES[j]); }
      }
    }
  ...
  function startGame(levelKey) {
    if (!STORIES.length) {
      viewEl.textContent = '序列库缺失，请检查 stories.js';
      return;
    }
    newRound(levelKey);
    renderGameView();
