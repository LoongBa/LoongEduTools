/* ============================================================
   找相同 — 元素库（SVG 程序化生成，Chrome 61 基线经典脚本）
   ------------------------------------------------------------
   来源：益智/观察力/找不同/src/assets/scenes.js 复制改编（元素 draw 函数
   原样保留，完全兼容 { fill, variant } 参数）。
   说明：
   ...
   - 每个元素 = { id, x, y, s, rot, hitR, fill, palette, variantMax, draw }
     ...
     - draw(opt)：返回以 (0,0) 为中心的内层 SVG 字符串（可直接作卡片图标）；
       opt = { fill, variant }；fill 改主色、variant 切换外观变体
   - 新增 window.ELEMENTS：展平全部场景元素为单卡片元素池（按 id 去重），
     找相同每轮随机抽 1 个元素 + 生成若干差异变体作干扰项
...
(function () {
  ...
  /* ---------- 元素中文名（找相同展示/文档用） ---------- */
  var ELEM_NAMES = {
    sun: '太阳', cloud: '云朵', boat: '小船', fish: '小鱼', starfish: '海星',
    shell: '贝壳', crab: '螃蟹', seagull: '海鸥', lighthouse: '灯塔', palm: '椰子树',
    tree: '大树', flower: '花朵', mushroom: '蘑菇', butterfly: '蝴蝶', bird: '小鸟',
    squirrel: '松鼠', rabbit: '兔子', fox: '狐狸', owl: '猫头鹰',
    barn: '谷仓', cow: '奶牛', sheep: '绵羊', chicken: '小鸡', tractor: '拖拉机',
    apple: '苹果树', fence: '栅栏', windmill: '风车', duck: '鸭子',
    'star-sun': '星星太阳', planet: '星球', rocket: '火箭', astronaut: '宇航员',
    satellite: '卫星', star2: '星星', ufo: '飞碟', moon: '月亮', comet: '彗星',
    fish2: '小鱼', octopus: '章鱼', turtle: '海龟', coral: '珊瑚', seaweed: '海草',
    jellyfish: '水母', starfish2: '海星', bubble: '泡泡', whale: '鲸鱼',
    house: '房子', car: '小汽车', tree2: '大树', lamp: '路灯', cloud2: '云朵',
    sun2: '太阳', bicycle: '自行车', balloon: '气球', bird2: '小鸟',
    snowman: '雪人', snowflake: '雪花', pine: '松树', igloo: '冰屋', sled: '雪橇',
    penguin: '企鹅', scarf: '围巾', deer: '小鹿', mitten: '手套',
    flower2: '花朵', bee: '蜜蜂', butterfly2: '蝴蝶', snail: '蜗牛', watering: '水壶',
    fence2: '栅栏', sun3: '太阳', mushroom2: '蘑菇', ladybug: '瓢虫'
  };

  /* ---------- 元素池（找相同用：展平 + 按 id 去重） ---------- */
  var flat = [];
  var seen = {};
  var scenes = [ocean, forest, farm, space, undersea, city, winter, garden];
  for (var si = 0; si < scenes.length; si++) {
    var sc = scenes[si];
    for (var ei = 0; ei < sc.elements.length; ei++) {
      var e = sc.elements[ei];
      if (seen[e.id]) { continue; }
      seen[e.id] = true;
      flat.push({
        id: e.id,
        name: ELEM_NAMES[e.id] || e.id,
        fill: e.fill,
        palette: e.palette || [],
        variantMax: e.variantMax || 0,
        draw: e.draw
      });
    }
  }
  window.ELEMENTS = flat;
})();
