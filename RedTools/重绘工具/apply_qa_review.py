#!/usr/bin/env python3
"""一键应用质检导出 → 合并 册级审核记录.json → 重新生成报告（去 AI 中转）
...
合并规则（与质检报告导出逻辑对称，可覆盖、不重复）：
  ...
  - 导出来自哪一册（book 字段），只更新该册 _重绘图片素材/审核记录.json，其他册不受影响
"""
...
def book_to_review_path(book: str) -> Path:
    """'四年级_上册' → F:/_教材素材/.../四年级/上册/_重绘图片素材/审核记录.json"""
    grade, _, vol = book.partition("_")
    if not grade or not vol:
        sys.exit(f"❌ book 格式应为 年级_册次（如 四年级_上册），收到: {book}")
    return MAT / grade / vol / "_重绘图片素材" / "审核记录.json"
...
def main() -> None:
    ...
    review_path = book_to_review_path(book)
    book_rev = load_json(review_path) if review_path.exists() else {}
    changed = 0
    ...
    review_path.write_text(json.dumps(book_rev, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"✅ {book} 已合并 {len(reviews)} 页（{changed} 页有变更）→ {review_path}")
    subprocess.run([sys.executable, str(GEN)], check=True)
    print("✅ 质检报告已重新生成")
